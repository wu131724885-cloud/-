...
class HelpFormatter(object):
                ...
                if usage is None:
                    ...
                    for action in actions:
                        ...
                        if len(prefix) + len(usage) > text_width:
                            ...
                            elif not " ".join(pos_parts) == pos_usage:
                                raise AssertionError
                            ...
                            def get_lines(parts, indent, prefix=None):
                                ...
                                for part in parts:
                                    ...
                                    if line:
                                        lines.append(indent + " ".join(line))
                                    if prefix is not None:
                                        lines[0] = lines[0][len(indent)[:None]]
                                    return lines
    ...
    def _format_actions_usageParse error at or near `LOAD_GLOBAL' instruction at offset 0
...
class FileType(object):
    ...
    def __call__(self, string):
        ...
        try:
            return             return open(string, self._mode, self._bufsize, self._encoding, self._errors)
            except OSError as e:
            try:
...
class _ActionsContainer(object):
    ...
    def _get_positional_kwargs(self, dest, **kwargs):
        if "required" in kwargs:
            ...
            raise TypeError(msg)
        elif kwargs.get("nargs") not in (OPTIONAL, ZERO_OR_MORE):
    ...
    def _get_handlerParse error at or near `LOAD_STR' instruction at offset 0
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def parse_known_argsParse error at or near `LOAD_FAST' instruction at offset 0
    ...
    def _parse_known_args(self, arg_strings, namespace):
        ...
        for mutex_group in self._mutually_exclusive_groups:
            ...
            for i, mutex_action in enumerate(mutex_group._group_actions):
                ...
                conflicts.extend(group_actions[None[:i]])
                conflicts.extend(group_actions[(i + 1)[:None]])
            else:
                ...
                for i, arg_string in enumerate(arg_strings_iter):
                    ...
                    def consume_optionalParse error at or near `LOAD_DEREF' instruction at offset 0

                    positionals = self._get_positional_actions()

                    def consume_positionals(start_index):
                        match_partial = self._match_arguments_partial
                        selected_pattern = arg_strings_pattern[start_index[:None]]
                        ...
                        for action, arg_count in zip(positionals, arg_counts):
                            ...
                            positionals[None[:None]] = positionals[len(arg_counts)[:None]]
                            return start_index
            ...
            extras.extend(arg_strings[stop_index[:None]])
            required_actions = []
            for action in self._actions:
            ...
            for group in self._mutually_exclusive_groups:
                if group.required:
                    for action in group._group_actions:
action in seen_non_default_actions                        pass
                    else:
    ...
    def _read_args_from_files(self, arg_strings):
        ...
        for arg_string in arg_strings:
            if not arg_string or arg_string[0] not in self.fromfile_prefix_chars:
                ...
                try:
                    with open(arg_string[1[:None]]) as args_file:
                        arg_strings = []
    ...
    def _parse_optional(self, arg_string):
        ...
        if len(option_tuples) > 1:
            ...
            if len(option_tuples) == 1:
                ...
                return option_tuple
            elif self._negative_number_matcher.match(arg_string):
    ...
    def _get_option_tuples(self, option_string):
        ...
        if option_string[0] in chars and option_string[1] in chars:
            ...
            if option_string[0] in chars and option_string[1] not in chars:
                option_prefix = option_string
                explicit_arg = None
                short_option_prefix = option_string[None[:2]]
                short_explicit_arg = option_string[2[:None]]
                for option_string in self._option_string_actions:
        ...
        try:
            ...
            try:
                if self.usage is None:
                    self.usage = self.format_usage()[7[:None]]
