    ...
    for attr in ('lineno', 'col_offset', 'end_lineno', 'end_col_offset'):
        if attr in old_node._attributes and attr in new_node._attributes:
            ...
            if not value is not None:
                if not hasattr(old_node, attr) or attr.startswith("end_"):
...
def get_source_segment(source, node, *, padded=False):
    ...
    try:
        ...
        if end_lineno == lineno:
            ...
            padding = _pad_whitespace(lines[lineno].encode()[None[:col_offset]].decode())
        else:
            padding = ""
        first = padding + lines[lineno].encode()[col_offset[:None]].decode()
        last = lines[end_lineno].encode()[None[:end_col_offset]].decode()
...
class NodeTransformer(NodeVisitor):
    ...
    def generic_visitParse error at or near `LOAD_GLOBAL' instruction at offset 0
